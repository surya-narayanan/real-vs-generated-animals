"""
Sample 500 real animal images, generate similar images via fal.ai Nano Banana
text-to-image, then plot RGB histograms comparing real vs generated.
"""
...
OUTPUT_DIR = "/home/paperspace/gen_comparison/nano_banana"
N_SAMPLE = 500
SEED = 42
...
# ---- Sample N (same seed as FLUX for fair comparison) ----
...
# ---- Generate fully synthetic images via Nano Banana text-to-image ----
real_arrays = []
gen_arrays = []
...
for i, img_path in enumerate(sampled):
    ...
    # Generate from noise via Nano Banana text-to-image
...
print(f"Generation complete. Total time: {(time.time() - t_start)/60:.1f} min")
...
for c, (ax, name, color) in enumerate(zip(axes, channel_names, channel_colors)):
    ...
    ax.hist(gen_vals, bins=bins, density=True, alpha=0.5, color=color,
            ...
            histtype="step", label="Generated (Nano Banana)")
...
plt.tight_layout()
fig.savefig(os.path.join(OUTPUT_DIR, "rgb_histogram_nano_banana.png"))
print(f"Histogram saved to {OUTPUT_DIR}/rgb_histogram_nano_banana.png")
...
axes2[1, 0].set_ylabel("Nano Banana", fontsize=7)
plt.tight_layout()
fig2.savefig(os.path.join(OUTPUT_DIR, "side_by_side_grid_nano_banana.png"))
print(f"Grid saved to {OUTPUT_DIR}/side_by_side_grid_nano_banana.png")

# ---- Statistical tests ----
print("\n" + "=" * 60)
print("STATISTICAL TESTS: Real vs Nano Banana pixel distributions")
print("=" * 60)
